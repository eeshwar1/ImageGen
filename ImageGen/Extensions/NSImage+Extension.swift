...
extension NSImage {
    ...
    func getDominantColors() -> [NSColor] {
        ...
        if let cgImage = self.cgImage(forProposedRect: nil, context: nil, hints: nil) {
            ...
            do {
                let cgDominantColors = try DominantColors.dominantColors(image: cgImage)
                
                for color in cgDominantColors {
                    
                    if let nsColor = NSColor(cgColor: color) {
                        dominantColors.append(nsColor)
                    }
                }
            } catch let error {
                
                print("error getting dominant colors \(error)")
            }
        } else {
